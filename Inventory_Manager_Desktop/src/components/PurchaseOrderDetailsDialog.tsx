...
export const PurchaseOrderDetailsDialog: React.FC<Props> = ({ open, onClose, orderId, onUpdate }) => {
  ...
  const [newItem, setNewItem] = useState({ productId: '', qty: '', cost: '' });
  ...
  const handleReceiveOrder = async () => {
    ...
    setActionLoading(true);
    try {
      await receivePurchaseOrder(orderId, parseInt(selectedWarehouse));
      onUpdate();
      onClose();
    } catch (err: any) {
  ...
  const onProductSelect = (product: any) => {
    if (!product) {
      setNewItem({ ...newItem, productId: '', cost: '' });
      return;
    }
