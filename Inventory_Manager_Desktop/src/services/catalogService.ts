...
export interface CreateLocationData {
  ...
  type: string;
  description?: string;
  // location_type mapping if needed, but the backend accepts "location_type"
  location_type?: string;
}

export const createLocation = async (data: CreateLocationData): Promise<Location> => {
  // Map type to location_type if needed, or user uses location_type directly
  const payload = {
    name: data.name,
